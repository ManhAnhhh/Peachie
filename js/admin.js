    ...
    return `
      <tr class="item${i}">
        ...
        <td style="display: flex; border: none; gap: 4px; height: 61px; border-bottom: 1px solid #ccc;">
          <div><button onclick="modifyProduct(${product.id},${i})" class="modify-product">Sửa</button></div>
          <div><button onclick="deleteProduct(${product.id},${i})">Xóa</button></div>
        </td>
      </tr>
    `
  })
...
document.querySelector('.modal-add-product').addEventListener('click', function () {
  document.querySelector('.modal-add-product').style.display = 'none';
});
document.querySelector('.modal-add-product form').addEventListener('click', function (e) {
...
const modifyProduct = (id, i) => {
  ...
  document.querySelector('.modal-modify-product').addEventListener('click', function () {
    document.querySelector('.modal-modify-product').style.display = 'none';
  });
  document.querySelector('.modal-modify-product form').addEventListener('click', function (e) {
    e.stopPropagation(); // prevent
  });
  ...
  product.sex == "male" ? document.getElementById('chkNamModify').checked = true : document.getElementById('chkNuModify').checked = true
      ...
      const newProducts = products.map(item => {
        if (item.id === id) {
          item.name = document.getElementById('txtNameModifyP').value,
            item.priceFake = (Number(document.getElementById('txtPriceFakeModifyP').value)),
            item.price = (Number(document.getElementById('txtPriceModifyP').value)),
            item.img = document.getElementById('txtImgModifyP').value,
            item.sex = (document.getElementById('chkNamModify').checked) ? "male" : "female",
            item.total = Number(document.getElementById('txtTotalModifyP').value),
            item.sold = 0,
...
const isCheckValueAdd = () => {
  ...
  if (!checkValue(txtNameP, "Tên sản phẩm không được bỏ trống. Mời bạn nhập lại!"))
    return false;
  if (!checkValue(txtImgP, "Link sản phẩm không được bỏ trống. Mời bạn nhập lại!"))
    return false;
  if (!checkNumber(priceFakeP, "Giá bán phải là số!"))
    return false;
  if (!checkNumber(priceP, "Giá khuyến mại phải là số!"))
    return false;
  if (!checkNumber(totalP, "Số lượng nhập không đúng định dạng!"))
    return false;
  return true;
}
...
const isCheckValueModify = () => {
  ...
  if (!checkValue(txtNameModifyP, "Tên sản phẩm không được bỏ trống. Mời bạn nhập lại!"))
    return false;
  if (!checkValue(txtImgModifyP, "Link sản phẩm không được bỏ trống. Mời bạn nhập lại!"))
    return false;
  if (!checkNumber(priceFakeModifyP, "Giá bán phải là số!"))
    return false;
  if (!checkNumber(priceModifyP, "Giá khuyến mại phải là số!"))
    return false;
...
if (productsCart == null) {
  ...
  totalCart = productsCart.reduce((total, item) => total + (item.quantity * 10) * item.price, 0);
}
...
tBodyContent = productsCart.map((item, index) => {
  ...
  const tonKho = (item.quantity * 10 > item.total) ? 0 : item.total - (item.quantity * 10);
  return `
    <tr>
      <td>${index + 1}</td>
      <td>${item.name}</td>
...
document.getElementById('btnExport').addEventListener("click", function () {
  // console.log(selectedOption);
  exportToExcel(`Báo cáo doanh thu ${selectedOption}`, 'Tháng 12', 'modal-table')
});
